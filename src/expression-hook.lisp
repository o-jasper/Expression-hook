...
(defpackage :expression-hook
  ...
  (:nicknames :expr-hook)
  (:export expand expand-file expand-hook *expression-hook*
...
(defparameter *expression-hook* 'expand-hook
  "Hook that sees every s-expression.
Change it to scan and such. Note that it has to be able to\
 read any expression it comes across. Return the result of\
 EXPAND-HOOK at the end.")
...
(defun macro-hook-to-expression-hook (macro-hook)
  "Conversion from macro hook to expression hook.(No  env though)"
  (lambda (expr)
...
(defparameter *ignore-packages* ;TODO SBCL specific.
  (list :sb-impl :sb-int :sb-c :sb-pcl :sb-kernel :sb-loop :sb-vm :sb-di)
...
(def-base-macro let (let (&rest vars) &body body)
  (denest
   ...
   (dolist (v vars (return-from base-let `(,let (,@binds) ,@(e-list body))))
     (cond
       ((listp v) v
...
(def-base-macro (multiple-value-bind destructuring-bind with-slots)
    (mvb bind from &body body)
  `(,mvb ,bind ,(expand from) ,@(e-list body)))

(def-base-macro load-time-value (load-time-value form &optional read-only-p 
						 &rest stuff)
  (when stuff (warn "Extra stuff found; ~s" stuff))
...
(def-base-macro defmethod (defmethod name &rest rest)
  (assert (listp rest))
  (multiple-value-bind (rest way) (if (symbolp (car rest))
				    (values (cdr rest) (list(car rest)))
				    rest)
    (assert (listp rest))
    (destructuring-bind (args &body body) rest
...
(def-base-macro setq (setq &rest pairs)
  `(,setq ,@(denest
	     (summing (:onto k)) (collecting (:ret t))
	     (dolist (p pairs)
	       (collecting (if (= (mod k 2) 0) p (expand p)))
	       (summing 1)))))
...
(defun expand-file (from &key load-first)
  "Convenience function to scan files with. Takes as arguments:
files or streams, or lists of these."
  (typecase from
    ((or string pathname)
     (if (string= (directory-namestring "denest.lisp") "")
       (with-open-file (stream from)
	 (when load-first (load from))
	 (expand-file stream :load-first load-first))
       (error "Didn't want to add the dependency needed to\
 scan directories recursively. Suggest using cl:fad and \
 making a 'recursively do' and using that with this.")))
    (list
     (mapcar (rcurry #'expand-file :load-first t) from))
    (stream
