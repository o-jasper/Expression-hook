;;
;;  Copyright (C) 18-02-2012 Jasper den Ouden.
;;
...
(defun write-system
 ...
 apparently missing."))
    ;Do stuff if provided. (Bunch of stuff not supported)
      (destructuring-bind (&key asd-license license
				author authors maintainer maintainers
				asd-version version &allow-other-keys) keys
	(when-let (version (or asd-version version))
	  (format stream "~%  :version \"~a\"" version))
	(when-let (license (or asd-license license))
	  (format stream "~%  :license ~s" license))
	(when (or author authors)
	  (format stream "~%  :author \"~{~a~^, ~}\"" 
		  (remove nil (cons author authors))))
	(when (or maintainer maintainers)
